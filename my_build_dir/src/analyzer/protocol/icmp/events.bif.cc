...
EventHandlerPtr icmp_sent_payload; 
void BifEvent::generate_icmp_sent_payload(analyzer::Analyzer* analyzer, Connection* c, Val* icmp, StringVal* payload)
	{
	// Note that it is intentional that here we do not
	// check if ::icmp_sent_payload is NULL, which should happen *before*
	// BifEvent::generate_icmp_sent_payload is called to avoid unnecessary Val
	// allocation.
	...
	mgr.QueueEvent(::icmp_sent_payload, vl, SOURCE_LOCAL, analyzer->GetID(), timer_mgr, c);
	} // event generation
