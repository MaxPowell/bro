...
EventHandlerPtr http_connection_upgrade; 
void BifEvent::generate_http_connection_upgrade(analyzer::Analyzer* analyzer, Connection* c, StringVal* protocol)
	{
	// Note that it is intentional that here we do not
	// check if ::http_connection_upgrade is NULL, which should happen *before*
	// BifEvent::generate_http_connection_upgrade is called to avoid unnecessary Val
	// allocation.
	...
	mgr.QueueEvent(::http_connection_upgrade, vl, SOURCE_LOCAL, analyzer->GetID(), timer_mgr, c);
	} // event generation
