// This file is automatically generated from /home/ubuntu/bro/src/analyzer/protocol/dhcp/dhcp.pac.
...
#include "/home/ubuntu/bro/my_build_dir/src/analyzer/protocol/dhcp/dhcp_pac.h"
...
int Option_Info::Parse(const_byteptr const t_begin_of_data, const_byteptr const t_end_of_data, int t_byteorder)
	...
	switch ( value_case_index() )
		...
		case 3:
			...
			if ( t_begin_of_data + t_router_list__arraylength > t_end_of_data + 1 || t_begin_of_data + t_router_list__arraylength < t_begin_of_data )
				{
			...
			for (; t_router_list__elem__it < t_router_list__arraylength; ++t_router_list__elem__it)
				...
				if ( t_router_list__elem__dataptr + (4) > t_end_of_data || t_router_list__elem__dataptr + (4) < t_router_list__elem__dataptr )
					{
					// Handle out-of-bound condition
		...
		case 12:
			...
			if ( (t_begin_of_data + 1) + (t_host_name__size) > t_end_of_data || (t_begin_of_data + 1) + (t_host_name__size) < (t_begin_of_data + 1) )
				{
				// Handle out-of-bound condition
			...
			throw binpac::ExceptionInvalidStringLength("/home/ubuntu/bro/src/analyzer/protocol/dhcp/dhcp-protocol.pac:44", t_host_name_string_length);
		...
		default:
			...
			if ( (t_begin_of_data + 1) + (t_other__size) > t_end_of_data || (t_begin_of_data + 1) + (t_other__size) < (t_begin_of_data + 1) )
				{
				// Handle out-of-bound condition
			...
			throw binpac::ExceptionInvalidStringLength("/home/ubuntu/bro/src/analyzer/protocol/dhcp/dhcp-protocol.pac:45", t_other_string_length);
...
int DHCP_Message::Parse(const_byteptr const t_begin_of_data, const_byteptr const t_end_of_data, ContextDHCP * t_context)
	...
	if ( t_dataptr_after_file + (4) > t_end_of_data || t_dataptr_after_file + (4) < t_dataptr_after_file )
		{
		// Handle out-of-bound condition
