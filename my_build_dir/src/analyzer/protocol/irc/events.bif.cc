...
EventHandlerPtr irc_starttls; 
void BifEvent::generate_irc_starttls(analyzer::Analyzer* analyzer, Connection* c)
	{
	// Note that it is intentional that here we do not
	// check if ::irc_starttls is NULL, which should happen *before*
	// BifEvent::generate_irc_starttls is called to avoid unnecessary Val
	// allocation.
	...
	mgr.QueueEvent(::irc_starttls, vl, SOURCE_LOCAL, analyzer->GetID(), timer_mgr, c);
	} // event generation
