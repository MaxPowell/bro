...
EventHandlerPtr dns_CAA_reply; 
void BifEvent::generate_dns_CAA_reply(analyzer::Analyzer* analyzer, Connection* c, Val* msg, Val* ans, bro_uint_t flags, StringVal* tag, StringVal* value)
	{
	// Note that it is intentional that here we do not
	// check if ::dns_CAA_reply is NULL, which should happen *before*
	// BifEvent::generate_dns_CAA_reply is called to avoid unnecessary Val
	// allocation.
	...
	mgr.QueueEvent(::dns_CAA_reply, vl, SOURCE_LOCAL, analyzer->GetID(), timer_mgr, c);
	} // event generation
